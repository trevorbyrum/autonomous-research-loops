...
- **D-34e (2026-09-08)** Round-5 review resolutions (Astra: FAIL, 5 findings, `report-round5.md` — every finding addressed): historical (unmarked) delegation windows can never consume events OWNED by any marked segment (double-count fixture: one 42-token invocation is 42, never 84) and use an exclusive end (same-second boundary ambiguity becomes an unknown allocation, never duplication); the repeat query uses the SAME status-aware dispatch semantics as the aggregates (HTTP 400/405/422 dispatches count toward repeats; only status-NULL broker refusals are excluded); completion ownership is pinned AT THE ROW BOUNDARY — each durable row's completion status must match its own attempt's (query, hop), so the 1↔2/3↔4 forwarding mutation fails wherever it wraps calllog.complete (verified by running the exact mutation: CAUGHT); the health column renders the search-cache count with its meaning; the tokenless health stamps EFFECTIVE lane knobs with execution's own max(1, …) clamp.
- **D-34d (2026-09-08)** Round-4 review resolutions (Astra: FAIL, 6 findings, `report-round4.md` — every finding addressed): delegation attribution moved from timestamps to FILE ORDER between the runner's iteration markers (append-only under flock), so same-second boundary events can never be discarded and a marked segment with zero launches is a true zero; unmarked success-only history is a SUBTOTAL with coverage unknown, rendered as a lower bound, never complete coverage; broker refusals are `refused AND status IS NULL` — HTTP 400/405/422 dispatches (also classed refused) stay in calls/credits/repeats, and iterations holding only crashed attempt rows keep their charged credits visible (`attempts_open` joins the no-rows test); tokenless health reads the cache's OWN expiry-aware `stats()` aggregates (the prior attribute read was always zero), a failed planner-estimate rolls back UNDER the cache lock so telemetry can never leave the shared transaction aborted, and the health stamp carries the gateway's lane-concurrency knobs for per-iteration configuration context; the runner's result records worker + delegate command and the report renders both; internal citations are excluded BEFORE the flagged counter (they contribute to neither number); the concurrent audit pin now asserts per-thread COMPLETION OWNERSHIP (each completion targets the attempt its own thread opened — a 1↔2 swap fails) and commit-at-transport (the dispatching thread's own attempt row must be in the connection's committed set when transport runs — a suppressed single commit fails), with the scripted connection tracking pending→committed transaction state.
