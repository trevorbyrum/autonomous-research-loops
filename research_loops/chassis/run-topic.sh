...
if [[ -n "${RESEARCH_LOOP_AGENT_SECONDARY:-}" ]]; then
  AGENT_NOTE=" DELEGATION: use the packaged managed wrapper: python3 ${CHASSIS}/managed-delegate.py secondary ${TOPIC_DIR} '<task>' for discovery, librarian work, extraction, proposal advocacy, and citation verification. Each verification is a fresh invocation distinct from the citation-producing invocation. Use python3 ${CHASSIS}/managed-delegate.py primary ${TOPIC_DIR} '<task>' for the authorized fresh counter-argument seat and its permitted repair assessment. These commands use the configured station secondary and primary records; do not add model flags or invoke a profile ID as a command. The primary retains final judgment (see CONTRACT-CORE.md step 4). Use no other delegate model or native Agent/Task intermediary. Delegate invocations run for MINUTES: issue the wrapper as ONE single foreground command and set that command's own shell timeout to at least 900 seconds (900000 ms) -- never launch it with '&', nohup, disown, or any other backgrounding, and never split the call into a launch step followed by separate polling/sleep/check commands. A harness tears down a backgrounded child's session the moment the visible command returns, silently killing it with no output and no error -- this is a confirmed platform limitation, not a timeout to work around by polling. A harness-killed or session-torn-down invocation is a capability failure, never searched-empty."
  if [[ -z "${RESEARCH_LOOP_MANAGED_SECONDARY_PROFILE:-}" ]]; then
    AGENT_NOTE=" DELEGATION: invoke ${RESEARCH_LOOP_AGENT_SECONDARY} with its configured default for discovery, librarian work, extraction, proposal advocacy, and citation verification. Each verification is a fresh invocation distinct from the citation-producing invocation. The authorized counter seat and permitted repair assessment use a fresh primary-class invocation through the wrapper with --model gpt-5.6-terra. The primary retains final judgment. Use no other delegate model or native Agent/Task intermediary. Issue the wrapper as ONE single foreground command and set that command's own shell timeout to at least 900 seconds (900000 ms) -- never launch it with '&', nohup, disown, or any other backgrounding, and never split the call into a launch step followed by separate polling/sleep/check commands. A harness tears down a backgrounded child's session the moment the visible command returns, silently killing it with no output and no error -- this is a confirmed platform limitation, not a timeout to work around by polling. A harness-killed or session-torn-down invocation is a capability failure, never searched-empty."
  fi
fi
