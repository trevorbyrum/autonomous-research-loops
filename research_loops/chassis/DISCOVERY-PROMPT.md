...
6. Scope traces to the operator's stated intent (in scoped mode: only check the contract renders the fixed frame faithfully — never whether the frame is right).

Do, in order:
1. CRITERIA CHECK: evaluate DRAFT-TOPIC.md and DRAFT-AUTHORITY.md against every criterion above; record findings.
2. If QA mode is `scoped`, STOP HERE and write outputs (step 5–6) with only the criteria findings and your questions: the operator's frame is fixed; no topic-space mapping, no scope proposals beyond what the criteria require.
